...
export class SensorsHandler extends HandlerBase {
  ...
  async getServices(): Promise<BaseAccessory[]> {
    ...
    const sensors = await this.getSensors() as SensorInfo[];

    this.log.info('SensorsHandler: received (' + sensors?.length + '): \'' + JSON.stringify(sensors) + '\'');

    for (let i = 0; i < sensors.length; i++) {
      const item = sensors[i] as SensorInfo;
  ...
  private async getSensors(): Promise<SensorInfo[]> {
    try {
      const response = await got('http://' + this.hostname + ':' + this.port + '/api/4/sensors', { throwHttpErrors: false });
      ...
      if (response.statusCode === 404) {
        ...
        const responsev3 = await got('http://' + this.hostname + ':' + this.port + '/api/3/sensors');
  ...
  async updateServices(): Promise<void> {
    ...
    for (let i = 0; i < sensors.length; i++) {
      ...
      if (sensor.type === 'temperature_core') {
        ...
        const existendItem = this.sensors.find(a => (a as TemperatureAccessory).Name === (this.prefix + ' ' + sensor.label));
